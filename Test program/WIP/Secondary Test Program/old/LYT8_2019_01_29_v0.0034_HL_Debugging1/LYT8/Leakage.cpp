...
Pulse pulse;
	...
	wait.delay_10_us(50);
	mux_20->open_relay(MUX_3_2);	//Connect Pullup R to TS pin.
	mux_20->open_relay(MUX_1_3);	//Connect Pullup R to UV pin.
	//Connect DSM CLK & Data to primary pins: TS & UV
	mux_14->open_relay(MUX_1_1);
	mux_14->open_relay(MUX_1_3);
	mux_14->open_relay(MUX_1_2);
	...
	FW_ovi3->set_voltage(FW_ch, -0.1, VOLT_5_RANGE);
	FW_ovi3->set_meas_mode(FW_ch, OVI_MEASURE_CURRENT);
	wait.delay_10_us(500);
	ILL1_FW = FW_ovi3->measure_average(25);

	FW_ovi3->set_voltage(FW_ch, 5.0, VOLT_5_RANGE);
	wait.delay_10_us(500);
	ILH2_FW = FW_ovi3->measure_average(25);

	FW_ovi3->set_current(FW_ch, 300e-6, RANGE_300_UA);
	FW_ovi3->set_voltage(FW_ch, 3.3, VOLT_5_RANGE);
	wait.delay_10_us(500);
	...
	VR_dvi->set_current(VR_ch, 300e-6, RANGE_300_UA);	
	VR_dvi->set_voltage(VR_ch, 15, VOLT_50_RANGE);
	delay(10);
