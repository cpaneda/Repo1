...
Pulse pulse;
	...
	// E0 //
	WordArray_E0[0]  = g_S_TrimRegister[16];
	WordArray_E0[1]  = g_S_TrimRegister[17];
	WordArray_E0[2]  = g_S_TrimRegister[18];
	WordArray_E0[3]  = 0; //g_S_TrimRegister[19]; T[19] = 0
	WordArray_E0[4]  = 0; //g_S_TrimRegister[20]; T[20] = 0
	WordArray_E0[5]  = g_S_TrimRegister[21];
	WordArray_E0[6]  = g_S_TrimRegister[22];
	WordArray_E0[7]  = g_S_TrimRegister[23];
	WordArray_E0[8]  = g_S_TrimRegister[24];
	WordArray_E0[9]  = g_S_TrimRegister[25];
	WordArray_E0[10] = g_S_TrimRegister[26];
	WordArray_E0[11] = g_S_TrimRegister[27];
	WordArray_E0[12] = g_S_TrimRegister[28];
	WordArray_E0[13] = g_S_TrimRegister[29];
	WordArray_E0[14] = g_S_TrimRegister[30];
	WordArray_E0[15] = g_S_TrimRegister[31];

	int BIN_2_Dec_Num = Convert_BIN_2_Dec(WordArray_E0);
	...
	TrimRegisterTemp[19] = 0;
	TrimRegisterTemp[20] = 0;
	Program_Trim_Register(TrimRegisterTemp);
	...
	for (i=0; i<64; i++)
	{
		temp_1 = (Fosc_pt_S * (1 + (Fosc_S_TrimWt[i]/100)) -  Fosc_Target_S1);
	...
	Fosc_TrCode_S = smallest_diff_idx;
	...
	{
		Fosc_BitCode_S1 = -1*Fosc_BitCode_S1 + 31; 
	}
	

	//Update WordArray.
	WordArray[0]        = g_S_TrimRegisterTemp[64]; 
	WordArray[1]        = g_S_TrimRegisterTemp[65]; 
	WordArray[2]        = g_S_TrimRegisterTemp[66]; 
	WordArray[3]        = g_S_TrimRegisterTemp[67]; 
	WordArray[4]        = g_S_TrimRegisterTemp[68]; 
	WordArray[5]        = g_S_TrimRegisterTemp[69]; 

	//Update secondary trim register array for programming later.
	g_S_TrimRegister[64] = g_S_TrimRegisterTemp[64]; 
	g_S_TrimRegister[65] = g_S_TrimRegisterTemp[65]; 
	g_S_TrimRegister[66] = g_S_TrimRegisterTemp[66]; 
	g_S_TrimRegister[67] = g_S_TrimRegisterTemp[67]; 
	g_S_TrimRegister[68] = g_S_TrimRegisterTemp[68]; 
	g_S_TrimRegister[69] = g_S_TrimRegisterTemp[69]; 
...
//Monitor 100Khz switching on HSG pin.	
	...
	PiDatalog(func, A_Fosc_target_S,     Fosc_Target_S1,           26, POWER_KILO);
	...
	PiDatalog(func, A_Eetr64_VCO0_S,    g_S_TrimRegister[64],    26, POWER_UNIT);
	PiDatalog(func, A_Eetr65_VCO1_S,    g_S_TrimRegister[65],    26, POWER_UNIT);
	PiDatalog(func, A_Eetr66_VCO2_S,    g_S_TrimRegister[66],    26, POWER_UNIT);
	PiDatalog(func, A_Eetr67_VCO3_S,    g_S_TrimRegister[67],    26, POWER_UNIT);
	PiDatalog(func, A_Eetr68_VCO4_S,    g_S_TrimRegister[68],    26, POWER_UNIT);
	PiDatalog(func, A_Eetr69_VCO5_S,    g_S_TrimRegister[69],    26, POWER_UNIT);
