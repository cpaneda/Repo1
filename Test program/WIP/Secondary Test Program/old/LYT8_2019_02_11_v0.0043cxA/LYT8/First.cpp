...
void First(test_function& func)
{
	...
	for(i=0;i<=80;i++)
	{
		g_P_TrimRegister[i]     = 0;
		g_S_TrimRegister[i]     = 0;
		g_P_TrimRegisterTemp[i] = 0;
	...
	theLimitSetName >> Part_ID_LimitSet  >> OperCode >> dummy >> dummy;
	
	// Set TempOpcode  //
	if (!strnicmp( OperCode, "4200", 4 ))
		TempOpcode = 4200;						
	if (!strnicmp( OperCode, "4250", 4 ))	
		TempOpcode = 4250;						
	if (!strnicmp( OperCode, "4500", 4 ))	
		TempOpcode = 4500;						
	if (!strnicmp( OperCode, "4300", 4 ))
		TempOpcode = 4300;						
	if (!strnicmp( OperCode, "4400", 4 ))
		TempOpcode = 4400;						
	if (!strnicmp( OperCode, "2400", 4 ))
		TempOpcode = 2400;
	...
	g_p_Char_Enable   = ours->CHAR_Enable;
	g_p_Rel_Enable    = ours->REL_Enable;
	...
	g_p_Trim_Enable = ours->TRIM_Enable;
	g_p_TstTime_Enble = ours->TestTime_Enable;
	...
	{
		g_p_Trim_Enable =0;  // Do not trim golden units. //
	}
...
*/
	...
	PiDatalog(func, A_p_Char_Enable, g_p_Char_Enable, 31, POWER_UNIT);
	PiDatalog(func, A_p_Rel_Enable, g_p_Rel_Enable, 31, POWER_UNIT);



	PiDatalog(func, A_p_Trim_Enable, g_p_Trim_Enable, 31, POWER_UNIT);
