...
pulse.do_pulse();
			...
			//Primary_Die_ID = DSM_I2C_Pri_Read(0x00);  //Read Pri-Silicon DieID from Reg Addr 0x00 using SlaveID 0x1C
		...
		if(1)
		{
			...
			//Read data out from EEPROM registers 0xC0,2,4,6,8
			pE0_data = DSM_I2C_Read(0xC0);
			//---- Differentiate ERASE and TRIMMED UNITS ------
	...
	else
	{
		...
		else
		{
			...
			pE2_data = DSM_I2C_Read(0xC2);
			pE4_data = DSM_I2C_Read(0xC4);
			pE6_data = DSM_I2C_Read(0xC6);
			pE8_data = DSM_I2C_Read(0xC8);		//Read back data of Register addr 0xE8 from 0xC8 register address through READ_ADDR 0x00
		...
		//-----------------------------------------------------------------------------------------------------------------
		// ERASE if "g_RTR_Enable" or "g_P_ERASE_Enable" and must be at OPCode 4200 *START
		//---------------------------------------------------------------------------------
		if( g_OPCODE==4200 && Pri_Untrimmed==1 )
		{
			EEPROM_ERASE_P(g_vDrain_Erase_th_P);
		}
		...
		//-----------------------------------------------------------------------------------------------------------------
		// ERASE if "g_RTR_Enable" or "g_P_ERASE_Enable" and must be at OPCode 4200 *END
		//---------------------------------------------------------------------------------
