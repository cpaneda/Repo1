...
// ZTMC_Dsbl_CConB (disable ccBoostOff) & HSG Open Drain to reduce noise.
     //0x00 0x60 write 0x20 0x00
	DSM_I2C_Write('w', 0x60, 0x8020);
	...
	// E2 //
	startbit = 16;
	WordArray_Assigned(WordArray_E2, startbit);
...
//pulse.do_pulse();
	...
	BFreq_TrCode_S = smallest_diff_idx;
...
////////
	//Update WordArray.
	WordArray[12]        = g_S_TrimRegisterTemp[12]; 
	WordArray[13]        = g_S_TrimRegisterTemp[13]; 
	WordArray[14]        = g_S_TrimRegisterTemp[14]; 
	

	//Update secondary trim register array for programming later.
	g_S_TrimRegister[12] = g_S_TrimRegisterTemp[12]; 
	g_S_TrimRegister[13] = g_S_TrimRegisterTemp[13]; 
	g_S_TrimRegister[14] = g_S_TrimRegisterTemp[14]; 
...
DSM_set_I2C_clock_freq(DSM_CONTEXT, 300);
    ...
    for(i=0;i<80;i++)
	{
		TrimRegister[i] = g_S_TrimRegister[i];
	}
	...
	Program_Trim_Register(TrimRegister);
