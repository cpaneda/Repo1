...
Pulse pulse;
	...
	Close_relay(K1_IS_SPI_TB);
	Close_relay(K3_IS_SPI_IB);       //Connect Resistor divider on IS buffer Module.
	Open_relay(K5_IS_SPI_IB);       //Connect IS buffer output to IS pin.
	...
	IS_dvi2k->set_current(IS_ch, 2e-3, RANGE_20_MA);
	IS_dvi2k->set_voltage(IS_ch, 0.5, VOLT_1_RANGE);
	wait.delay_10_us(10);
...
pulse.do_pulse();
	...
	float delta = ccPassFEToff_TH - ccBoostOff_TH;
	delay(1);
