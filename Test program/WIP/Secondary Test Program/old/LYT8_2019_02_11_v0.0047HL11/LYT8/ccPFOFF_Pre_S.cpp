...
Pulse pulse;
	...
	//ZOpenDrain<1> & ZTMC_Dsbl_DaliCk2 & ZTMC_ISfltTst
	//0x00 0x60 write 0x02 0x50 
	DSM_I2C_Write('w', 0x60, 0x5002);
	//DSM_I2C_Write('w', 0x60, 0x0002);
	DSM_set_I2C_clock_freq(DSM_CONTEXT, 300);
...
pulse.do_pulse();
	...
	////while (j <0.65)
	////	{
	////		IS_dvi2k->set_voltage(IS_ch, j, VOLT_1_RANGE);
	////		wait.delay_10_us(20);
	////		j=j+0.001;
	////	}
...
IS_dvi2k->set_voltage(IS_ch, 0.8, VOLT_2_RANGE);
              	...
              	for(i = 0; i<1;i++)
	{
		...
		////while (j <0.65)
		////{
		////	IS_dvi2k->set_voltage(IS_ch, j, VOLT_2_RANGE);
		////	wait.delay_10_us(2);
		////	j=j+0.01;
		////}*/
		...
		////IS_dvi2k->set_voltage(IS_ch, 0.8, VOLT_1_RANGE);
		////wait.delay_10_us(10);*/

		ccBoostOff_TH = search_direction(1);
...
if(0)
{
		...
		for(i=0;i<20;i++)
		{
			IS_dvi2k->set_voltage(IS_ch, 1, VOLT_1_RANGE);
			wait.delay_10_us(10);
			IS_dvi2k->set_voltage(IS_ch, 0.3, VOLT_1_RANGE);
			wait.delay_10_us(10);
		}
		...
		ccPassFEToff_TH = search_direction(0);

		delta = delta + ccPassFEToff_TH - ccBoostOff_TH;
}
